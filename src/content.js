...
// 조작 OSD 종류별 표시/위치(중심 기준 %). 볼륨=전체 화면, 되감기=왼쪽 절반, 앞으로=오른쪽
// 절반 기준(x 0~100). 기본값은 audioMixer 기본과 동일. settings 에서 조절.
const ACTION_OVERLAY_POS_KEY = "cheeseActionOverlayPos";
const ACTION_OVERLAY_POS_DEFAULT = {
  volume: { on: true, x: 50, y: 50 },
  rewind: { on: true, x: 30, y: 50 },
  forward: { on: true, x: 70, y: 50 },
};
let actionOverlayPos = normalizeActionOverlayPos(null);
function normalizeActionOverlayPos(raw) {
  const out = {};
  for (const k of ["volume", "rewind", "forward"]) {
    const d = ACTION_OVERLAY_POS_DEFAULT[k];
    const s = raw && typeof raw === "object" ? raw[k] : null;
    const nx = Number(s?.x);
    const ny = Number(s?.y);
    out[k] = {
      on: s && typeof s.on === "boolean" ? s.on : d.on,
      x: Number.isFinite(nx) ? Math.min(100, Math.max(0, nx)) : d.x,
      y: Number.isFinite(ny) ? Math.min(100, Math.max(0, ny)) : d.y,
    };
  }
  return out;
}
...
const FOLLOW_PREVIEW_ALWAYS_ELAPSED_KEY = "cheeseFollowPreviewAlwaysElapsed";
let followPreviewAlwaysViewers = false;
let followPreviewAlwaysElapsed = false;
...
// ── 전용 팔로잉 목록(sbFollowCustom) 설정/상태 ──────────────────────────────
const CUSTOM_FOLLOW_SORT_KEY = "cheeseFollowCustomSort";
...
// 즐겨찾기 내 별도 정렬은 '커스텀 순서(custom)'도 허용한다(일반 정렬엔 없음).
...
// 헤더 미니 네비 항목 → 대응하는 사이드바 개별 숨김 플래그(featureFlags 키). 사이드바가
// 보여도, 사이드바에서 이 항목을 개별 숨겼고 헤더 바로가기가 ON이면 헤더에 보여준다.
const HEADER_NAV_SIDEBAR_HIDE_FLAG = {
  "/lives": "sbLives",
  "/clips": "sbClips",
  "/category": "sbCategory",
  "/schedule": "sbSchedule",
...
// 현재 재생 영상이 '세로'(videoHeight > videoWidth)인지 <html> 클래스로 표시한다.
// seek preview 실제시각 라벨·댓글 마커 툴팁이 좁은 세로 preview 에 맞게 CSS 를 달리 적용.
function updateVodPortraitClass() {
...
function updateSeekPreviewRealtime() {
  ...
  // 는 두 줄 구조에서 부정확할 수 있어, 우리 span 을 뺀 나머지 자식의 텍스트만 합친다.
  const existing = timeEl.querySelector(`.${SEEK_PREVIEW_REALTIME_CLASS}`);
  let baseText = "";
  timeEl.childNodes.forEach((n) => {
...
function videoInfoDatesHtml() {
  ...
  if (seekPreviewState.publishAt) {
    parts.push(
      `등록일 : ${escapeHtml(formatKstClock(seekPreviewState.publishAt))}`,
    );
  }
...
function videoInfoTriggerFromTarget(target) {
  ...
  const hasDateLabel = [
    ...host.querySelectorAll(VIDEO_INFO_LABEL_SELECTOR),
  ].some((l) => l.textContent.includes("등록일"));
...
function bindVideoInfoLabel() {
  ...
  document.addEventListener("mouseover", onVideoInfoMouseOver, {
    passive: true,
  });
...
async function fetchChannelLogPowerContent(channelId) {
  try {
    const res = await fetch(`${LOGPOWER_CHANNEL_BASE}/${channelId}/log-power`, {
      credentials: "include",
    });
...
function startLogPowerClaimTimer() {
  ...
  // hidden 스킵 없음: 1시간 시청 보상(WATCH_1_HOUR) claim은 탭이 백그라운드일 때도
  // 처리돼야 1시간 타이머가 제때 시작된다(백그라운드 throttle은 있어도 동작은 함).
...
// ══ 통나무파워 적립 추적 + 1시간 타이머 표시(적립은 background 위임) ════════════
// 5분 주기 적립 판정은 background(chrome.alarms + storage.session)에서 한다 — 탭이
// 백그라운드여도 동작하고 SPA 이동에도 상태가 유지된다. content는 시작/복원만
...
// 라이브 종료 broadcast 처리: 그 채널의 적립·1시간 타이머 표시 정리.
function clearLogPowerLiveStates(channelId) {
  if (logPowerWatchActiveChannelId === channelId) {
    logPowerWatchActive = false;
  }
...
function renderWatchHourTimer() {
  ...
  if (
    !logPowerHourEndsAt ||
    logPowerHourChannelId !== getCurrentLiveChannelId()
  ) {
...
function handleOtherTabLogPowerToast(message) {
  if (
    !featureFlags.chatLogPowerToast ||
    !featureFlags.chatLogPowerToastOtherTabs
  )
    return;
...
function renderCommentMarkerPreviewTooltip(seconds, attempt = 0) {
  ...
  updateVodPortraitClass(); // 세로 방송이면 툴팁을 좁은 preview 폭에 맞추기 위해
...
function resetChatAsideWidth(aside) {
  ...
  // 강제했던 입력창 높이 원복. 단, 그냥 height 인라인을 지우면 치지직 textarea의
  // auto-grow가 마지막 계산값(max-height 60px에 걸린 값)으로 height를 채워 60px로
  // 고정돼 버린다(빈 상태인데도). 그래서 빈/비포커스면 기본 40px로 명시 복원하되,
  // !important는 빼서 입력/포커스 시 치지직이 정상적으로 다시 키울 수 있게 둔다.
  const textarea =
    aside.querySelector("textarea[placeholder*='채팅을 입력해주세요']") ||
    aside.querySelector("textarea[class*='_input_']") ||
    aside.querySelector("textarea[placeholder*='채팅']");
  if (textarea instanceof HTMLTextAreaElement) {
    const empty = String(textarea.value || "").length === 0;
...
function restoreVodMorePosition() {
  ...
  if (!right) return;
  if (
    vodMoreOrigParent instanceof HTMLElement &&
    vodMoreOrigParent.isConnected
  ) {
...
// ── 채팅창 접힘 상태 유지 ────────────────────────────────────────────────────
// 치지직 채팅 aside 는 접으면 '_is_folded_' 클래스가 붙고 표시가 사라진다. 접기/펼치기
// 는 aria-label="채팅 접기" 버튼 클릭으로 토글된다. 사용자가 접으면 상태를 저장하고,
// 라이브 진입 시 저장 상태와 현재 상태가 다르면 그 버튼을 클릭해 맞춘다(React-safe —
// 우리가 aside 를 직접 조작하지 않고 네이티브 토글만 트리거).
// 채팅 접기/펼치기 토글 버튼. 실측 결과 상태별로 버튼이 다르다:
...
let chatFoldSaveTimer = 0;
let chatFoldRestoreUntil = 0; // 이 시각까지는 '복원 중' — 옵저버 저장 억제 + 되돌림 재교정
let chatFoldWant = null; // 복원 목표 상태(true=접힘). 복원 기간 동안 이 값으로 재교정.
let chatFoldEnforceTimer = 0;
let chatFoldStableCount = 0; // 목표 상태가 연속 안정된 횟수(안정화 판정)
let chatFoldUserClickBound = false; // 사용자 접기/열기 클릭 감지 리스너 바인딩 여부

function getLiveChatAside() {
  return document.querySelector("aside#aside-chatting");
}
// 사용자가 채팅 접기/열기 버튼을 '직접' 클릭하면(우리 복원 클릭이 아니라), 그 의사가
// 우선이다. 복원 기간 중이라도 즉시 복원을 종료하고 enforce 를 멈춘 뒤 새 상태를 저장해,
// 사용자가 연 채팅을 우리가 다시 접어버리는 일을 막는다. 우리 프로그램적 click 은
// isTrusted=false, 실제 사용자 클릭은 isTrusted=true 라 구분된다. React 재렌더로 버튼이
// 바뀌어도 document 캡처 위임으로 안정적으로 잡는다.
function ensureChatFoldUserClickListener() {
  if (chatFoldUserClickBound) return;
  chatFoldUserClickBound = true;
  document.addEventListener(
    "click",
    (e) => {
      if (!chatFoldPersistOn) return;
      if (!e.isTrusted) return; // 우리 복원 click 은 무시
      // 접기 버튼(aria-label) 또는 펼치기 버튼(_folded_button_ / 텍스트 '채팅 (J)').
      const el = e.target?.closest?.(
        'button[aria-label="채팅 접기"], button[class*="_folded_button_"], button',
      );
      let btn = null;
      if (el) {
      ...
      if (!btn) return;
      handleChatFoldUserToggle();
    },
    true, // 캡처 단계: 치지직 핸들러보다 먼저 우리 의도를 확정
  );
  // 단축키 J(치지직 네이티브 채팅 접기/펼치기)는 클릭이 아니라 keydown 이라 위 클릭
  // 리스너로 못 잡는다. 한글 입력 상태에선 e.key 가 'ㅓ' 로 오므로(치지직도 그렇게
  // 동작하듯) 물리 키 기준인 e.code === "KeyJ" 로 언어 무관하게 판정한다.
  document.addEventListener(
    "keydown",
    (e) => {
      if (!chatFoldPersistOn) return;
      ...
      // 텍스트 입력 중(채팅 입력창 등)이면 J 는 글자 입력이지 단축키가 아니다.
      const t = e.target;
      if (
        t &&
        (t.isContentEditable ||
...
// 사용자가 직접 채팅을 접거나 펼침(클릭/단축키) → 복원 즉시 종료 + enforce 중단 +
// 마스킹 해제 + 새 상태 저장. 이후 우리가 다시 되돌리지 않도록 목표를 비운다.
function handleChatFoldUserToggle() {
  chatFoldRestoreUntil = 0;
  if (chatFoldEnforceTimer) {
    clearTimeout(chatFoldEnforceTimer);
    chatFoldEnforceTimer = 0;
  }
  chatFoldWant = null;
  document.documentElement.classList.remove(
    "cheese-chat-fold-restoring",
    "cheese-chat-fold-restoring-instant",
  );
  // 토글 반영(클래스 변경) 후 새 상태를 저장 — 다음 진입 복원 기준이 된다.
  setTimeout(() => {
    try {
      chrome.storage.local.set({ [CHAT_FOLD_STATE_KEY]: isChatFolded() });
    } catch {}
  }, 120);
}
function isChatFolded(aside) {
...
const AD_MARKER_SEL =
  "[class*='pzp-ad'], .pzp-ad-click-through-button, [data-role='adVideoContainerEl'], .vod_player_wrap, [data-role='skipBtn'], .ad_info_area, [aria-label*='광고']";
...
// "mm:ss"(또는 "hh:mm:ss") → 초. 파싱 실패 시 NaN.
function parseAdClock(txt) {
  if (!txt) return NaN;
  const parts = txt.trim().split(":").map(Number);
  if (parts.some((n) => !Number.isFinite(n))) return NaN;
  return parts.reduce((acc, n) => acc * 60 + n, 0);
}
// 광고가 재생 중이면 남은 초(총 재생시간 - 현재 재생시간)를 반환, 아니면 null.
// 광고 플레이어 DOM 의 curTimeLabel/durationLabel(예 00:15 / 00:58)로 계산한다.
// 광고 길이를 알면 복원 대기 상한을 딱 그만큼만 밀 수 있어 무한 대기를 피한다.
function adRemainingSeconds() {
  if (!isAdPlaying()) return null;
  const cur = parseAdClock(
    document.querySelector("[data-role='curTimeLabel']")?.textContent,
  );
  const dur = parseAdClock(
    document.querySelector("[data-role='durationLabel']")?.textContent,
  );
  // dur 이 0(00:00) 이면 아직 광고 메타데이터가 안 잡힌 초기 상태 → 시간 미상 취급.
  // 이때 (dur-cur)=0 으로 만료를 6초만 밀면 광고 도중 조기 종료되므로 NaN(보수적 20초).
  if (!Number.isFinite(cur) || !Number.isFinite(dur) || dur <= 0) return NaN;
  return Math.max(0, dur - cur);
}

// aside 접힘 클래스 변화를 감지해 저장(디바운스). 우리가 복원으로 토글한 직후 변화도
// 저장되지만, 값이 같으면 무해하고 다음 진입 복원에 그대로 쓰인다.
function ensureChatFoldObserver() {
  if (!chatFoldPersistOn) return;
  ensureChatFoldUserClickListener();
  const aside = getLiveChatAside();
  if (!aside) return;
  if (chatFoldObserver && chatFoldObserver._target === aside) return;
  if (chatFoldObserver) chatFoldObserver.disconnect();
  chatFoldObserver = new MutationObserver(() => {
    // 복원 기간(치지직 초기화가 채팅창을 되돌릴 수 있는 창) 동안엔 사용자 조작이 아니라
    // 우리 복원/치지직 되돌림이 원인이므로 저장하지 않는다. 되돌림 재교정은 아래 enforce.
    if (Date.now() < chatFoldRestoreUntil) return;
    if (chatFoldSaveTimer) return;
    // 입장/중간 광고 재생 중엔 치지직이 채팅창을 강제로 펼쳐두므로, 그 펼침을 사용자
    // 조작으로 오인해 저장하면 안 된다(다음 진입에 '펼침'으로 복원됨). 광고 중엔 저장 스킵.
    if (isAdPlaying()) return;
    chatFoldSaveTimer = window.setTimeout(() => {
      chatFoldSaveTimer = 0;
      if (isAdPlaying()) return;
      try {
        chrome.storage.local.set({ [CHAT_FOLD_STATE_KEY]: isChatFolded() });
  ...
  chatFoldObserver.observe(aside, {
    attributes: true,
    attributeFilter: ["class"],
  });
  chatFoldObserver._target = aside;
}
function stopChatFoldObserver() {
  if (chatFoldObserver) {
    chatFoldObserver.disconnect();
    chatFoldObserver = null;
  }
  if (chatFoldEnforceTimer) {
    clearTimeout(chatFoldEnforceTimer);
    chatFoldEnforceTimer = 0;
...
function applyChatFoldPersist() {
  ...
  const aside = getLiveChatAside();
  if (!aside) return; // 채팅 aside 아직 없음 → 다음 tick
  ensureChatFoldObserver();

  const key = location.pathname; // 채널(라이브)별 1회 복원
  if (chatFoldRestoredForKey === key) return;
  chatFoldRestoredForKey = key; // 중복 복원 방지(비동기 get 전에 선점)
  try {
    chrome.storage.local.get(CHAT_FOLD_STATE_KEY, (data) => {
      chatFoldWant = data?.[CHAT_FOLD_STATE_KEY] === true; // 목표: 접힘 여부
      // 치지직은 페이지 로드 시 채팅창을 자체 초기화(보통 펼침)한다. 우리 복원 click 이
      // 그 초기화보다 앞서면 되돌려진다. 그래서 '복원 기간' 동안 250ms 마다 현재 상태를
      // 확인해, 목표와 다르면(치지직이 되돌렸으면) 다시 토글해 굳힌다. 목표가 연속으로
      // 안정되면(enforce 참고) 기간 내라도 조기 종료한다. 상한은 넉넉히(20초) — 입장
      // 광고(pre-roll)가 재생되면 그동안 채팅/레이아웃이 불안정한데, 광고가 끝나고 채팅이
      // 안정될 때까지 마스킹을 유지해야 광고 후 깜빡임을 막는다.
      chatFoldRestoreUntil = Date.now() + 20000;
      // 목표가 '접힘'이면, 복원 기간의 깜빡임(치지직이 잠깐 펼침)을 CSS 로 즉시 가린다.
      // 복원이 끝나면 마스킹을 제거해 정상 접힘 상태만 남긴다. 목표가 '펼침'이면 마스킹
      // 불필요(현재 펼쳐진 그대로 두면 됨).
      const root = document.documentElement;
      if (chatFoldWant === true) {
        // 초기엔 transition 없이(-instant) 즉시 숨겨 '슬라이드 인' 없이 시작하고, 다음
        // 프레임에 -instant 만 떼어 이후 재교정/해제는 부드러운 슬라이드가 되게 한다.
        root.classList.add(
          "cheese-chat-fold-restoring",
          "cheese-chat-fold-restoring-instant",
        );
        requestAnimationFrame(() => {
          requestAnimationFrame(() =>
            root.classList.remove("cheese-chat-fold-restoring-instant"),
          );
        });
      } else {
...
// 복원 기간 동안 목표 접힘 상태를 유지(되돌림 재교정). 치지직은 로드 후 한 번이 아니라
...
function startChatFoldEnforce() {
  ...
  let reachedGoalOnce = false; // 목표(접힘)를 한 번이라도 달성했는지
  const TOGGLE_COOLDOWN_MS = 500; // 토글 직후 이 시간은 재토글 금지(반영 대기 + 깜빡임↓)
  const clearMask = () =>
    document.documentElement.classList.remove(
      "cheese-chat-fold-restoring",
      "cheese-chat-fold-restoring-instant",
  ...
  const tick = () => {
    ...
    // 무시한다. 그러니 억지로 접으려 하지 않고, 목표가 '접힘'이면 마스킹으로 가린 채
    // 광고가 끝날 때까지 대기한다(사용자 제안: 광고 중엔 자동 숨김처럼 처리). 광고 중엔
    // 복원 기간 만료로 종료하지 않도록 만료 시각도 계속 뒤로 민다.
    const adRemain = adRemainingSeconds();
    const now0 = Date.now();
    if (adRemain !== null) {
      // 광고 중엔 치지직이 채팅창을 강제로 '펼침'으로 되돌린다. 이때 마스킹
      // (translateX 슬라이드)은 aside 가 접혀(폭 0) 있을 때만 화면 밖에 놓이고,
      // 펼쳐진(정상 폭) aside 에 걸면 영상 위로 밀려 올라와 오버레이가 된다. 그래서
      // 광고 중 목표가 '접힘'이어도 aside 가 실제로 접혀 있을 때만 마스킹을 유지하고,
      // 펼쳐져 있으면 마스킹을 끈다(오버레이 방지). 접기 시도는 계속 하되(광고가
      // 무시할 수 있음) 광고가 끝나면 아래 정상 경로가 이어받아 접는다.
      const aAd = getLiveChatAside();
      const btnAd = getChatFoldToggleBtn();
      const foldedAd = aAd ? isChatFolded(aAd) : false;
      if (chatFoldWant === true && foldedAd) {
        document.documentElement.classList.add("cheese-chat-fold-restoring");
      } else {
        clearMask(); // 펼쳐진 상태 → 마스킹 걸면 오버레이되므로 해제
      }
      if (chatFoldWant === true && aAd && btnAd && !foldedAd) {
        // 접기 시도(광고가 무시할 수 있음). 쿨다운으로 왕복 토글 방지.
        if (now0 - lastToggleAt >= TOGGLE_COOLDOWN_MS) {
          btnAd.click();
          lastToggleAt = now0;
        }
      }
      chatFoldStableCount = 0;
      wasAd = true;
      // 광고 길이를 알면 (남은 초 + 6초 여유)만큼만 만료를 민다 → 광고 끝나면 곧
      // 만료돼 무한 대기하지 않는다. 길이 미상(NaN)이면 보수적으로 20초 민다.
      const extendMs = Number.isFinite(adRemain)
        ? (adRemain + 6) * 1000
        : 20000;
      chatFoldRestoreUntil = Date.now() + extendMs;
      chatFoldEnforceTimer = window.setTimeout(tick, 250);
      return;
    }
    // 광고가 방금 끝났으면(직전 틱=광고), 이제부터 정상 경로로 접어야 한다. 그런데
    // 광고 중 만료 시각이 짧게 남았을 수 있어(광고 종료 근처) 접기 전에 만료돼 버리면
    // 채팅이 펼쳐진 채 남는다. 그래서 광고 종료 직후 복원 기간을 넉넉히 리셋한다.
    if (wasAd) {
      wasAd = false;
      chatFoldRestoreUntil = Date.now() + 8000; // 광고 후 접기 완료 시간 확보
    }
    // 복원 기간이 끝났으면(광고도 아닌데 안정화 실패) 마스킹 해제하고 종료(무한 방지).
    ...
    const btn = getChatFoldToggleBtn();
    const atGoal = a && isChatFolded(a) === chatFoldWant;
    const now = Date.now();
    if (atGoal) {
      // 목표 달성. 이후 목표를 벗어나면 '사용자 조작'으로 보고 재교정하지 않도록 표시.
      reachedGoalOnce = true;
      chatFoldStableCount += 1;
      // 목표가 연속으로 안정되면 마스킹 제거 + 감시 종료(더는 되돌림 없음으로 판단).
      ...
      // 한 번 목표를 달성한 뒤 벗어남 = 사용자가 직접 펼침/접음(단축키 j·마우스 포함).
      // 사용자 의사를 존중해 재교정하지 않고 종료한다(다시 접어버리는 문제 해결).
      // 새 상태 저장은 observer 가 담당(복원 기간 밖이 되면 저장됨).
      clearMask();
      chatFoldEnforceTimer = 0;
      ...
      chatFoldStableCount = 0;
      // 쿨다운 안이면 재토글 금지(왕복 토글/깜빡임 완화). 마스킹이 가리므로 대기 무해.
  ...
  chatFoldEnforceTimer = window.setTimeout(tick, 0);
}
...
function applySidebarSections() {
  ...
  sections.forEach((nav) => {
    ...
    if (label.includes("팔로"))
      // 전용 팔로잉 목록이 켜져 있으면 팔로우 섹션을 숨기지 않는다(우리 목록을 그 안에
      // 주입하므로). 그 외엔 sbFollow(섹션 통째 숨김) 설정을 따른다.
      hidden = featureFlags.sbFollowCustom ? false : featureFlags.sbFollow;
    ...
    if (btn && !followCollapseRemoveTimer) {
      followCollapseRemoveTimer = window.setTimeout(() => {
        ...
        if (stillCollapsed && !shouldExpandFollow()) {
          document.querySelector(`.${FOLLOW_HEADER_COLLAPSE_CLASS}`)?.remove();
        }
      }, 600);
    }
    return;
...
function onHeaderFollowClick(event) {
  ...
  // fixed 로 뜬 미리보기가 남았다.
  dismissHeaderFollowHover();
  spaNavigate(href);
}

// ── 헤더 팔로우 툴팁 JS 제어 ────────────────────────────────────────────────
// :hover CSS 로 툴팁을 띄우면 클릭 이동 후 마우스 잔류·헤더 재렌더·자동숨김 복귀에서
// 유령/중복 툴팁이 떴다. 대신 pointer 위치로 '현재 실제 가리키는 아이템' 하나에만
// show 클래스를 부여한다. 클릭 이동 직후엔 억제해, 마우스가 헤더 밖으로 나갔다 특정
// 아이템에 다시 들어올 때까지 툴팁을 안 띄운다.
const HEADER_FOLLOW_SHOW_CLASS = "cheese-header-follow-item-show-tooltip";
let headerFollowTooltipSuppressed = false; // 클릭 이동 직후 억제 상태
let headerFollowTooltipShownItem = null; // 현재 show 부여된 아이템

function setHeaderFollowTooltipItem(item) {
  if (headerFollowTooltipShownItem === item) return;
  headerFollowTooltipShownItem?.classList.remove(HEADER_FOLLOW_SHOW_CLASS);
  headerFollowTooltipShownItem = item || null;
  item?.classList.add(HEADER_FOLLOW_SHOW_CLASS);
}

// pointer 위치의 헤더 팔로우 아이템에만 툴팁을 부여한다(억제 중이면 아무것도 안 띄움).
function onHeaderFollowTooltipPointerMove(e) {
  const container = document.getElementById(HEADER_FOLLOW_CONTAINER_ID);
  if (!container) {
    setHeaderFollowTooltipItem(null);
    return;
  }
  const overItem = e.target?.closest?.(".cheese-header-follow-item");
  const inContainer = overItem && container.contains(overItem);
  // 억제 해제 조건: 마우스가 헤더 컨테이너(팔로우 영역) 밖으로 나가면 억제를 푼다.
  if (headerFollowTooltipSuppressed) {
    ...
    const inside =
      ...
      e.clientY >= r.top &&
      e.clientY <= r.bottom;
    if (!inside) headerFollowTooltipSuppressed = false;
  }
  if (headerFollowTooltipSuppressed || !inContainer) {
    setHeaderFollowTooltipItem(null);
    return;
  }
  setHeaderFollowTooltipItem(overItem);
}
document.addEventListener(
  "pointermove",
  onHeaderFollowTooltipPointerMove,
  true,
);
// 키보드 포커스 접근성: 탭으로 헤더 팔로우 링크에 포커스가 가면 그 아이템 툴팁 표시.
document.addEventListener("focusin", (e) => {
  const container = document.getElementById(HEADER_FOLLOW_CONTAINER_ID);
  const item = e.target?.closest?.(".cheese-header-follow-item");
  if (container && item && container.contains(item)) {
    headerFollowTooltipSuppressed = false;
    setHeaderFollowTooltipItem(item);
  }
});
document.addEventListener("focusout", (e) => {
  const item = e.target?.closest?.(".cheese-header-follow-item");
  if (item && item === headerFollowTooltipShownItem) {
    setHeaderFollowTooltipItem(null);
  }
});

// 헤더 팔로우 아이템의 미리보기·커스텀 툴팁을 즉시 정리한다(클릭 이동 시).
function dismissHeaderFollowHover() {
  ...
  followPreviewOpenSuppressUntil =
    Date.now() + FOLLOW_PREVIEW_CLICK_SUPPRESS_MS;
  // 현재 툴팁 숨기고, 마우스가 헤더 밖으로 나갔다 다시 아이템에 들어올 때까지 억제.
  setHeaderFollowTooltipItem(null);
  headerFollowTooltipSuppressed = true;
}
...
function handleSidebarExpandTransition(sidebar) {
  ...
  if (expanded !== sidebarWasExpanded) {
    ...
    // ⚠ 전용 팔로잉 목록: 전환 시 즉시 재렌더 + harvest 가 아직 innerExpanded 를 못
    // 얻었으면(펼침 직후 원본 li 가 아직 안 그려짐) 다음 프레임에 한 번 더 시도해 UI 깨짐
    // 창을 최소화한다. sig 에 isSidebarExpanded 가 있어 값 변화 시에만 실제 DOM 을 바꾼다.
    if (featureFlags.sbFollowCustom) {
      ensureCustomFollowList();
      ensureCustomFollowCollapsedControls();
      // 원본 li 에 _is_expanded_ 가 몇 ms 늦게 붙어도 다음 프레임에 실시간 조회로 반영.
      requestAnimationFrame(() => {
        ensureCustomFollowList();
        ensureCustomFollowCollapsedControls();
      });
...
// 이 헤더 네비 항목을 실제로 헤더에 노출할지: 바로가기 ON 이고, (사이드바 전체 숨김 OR
// 이 항목이 사이드바에서 개별 숨김)일 때. 사이드바가 보이고 그 항목도 보이면 헤더에
// 중복 노출하지 않는다(사이드바에서 이미 접근 가능).
function isHeaderNavItemVisible(item) {
  if (!isHeaderNavShown(item.key)) return false;
  if (featureFlags.sidebar) return true; // 사이드바 전체 숨김 → 전부 노출
  const flag = HEADER_NAV_SIDEBAR_HIDE_FLAG[item.href];
  return flag ? featureFlags[flag] === true : false; // 개별 숨김 항목만
}
...
// 사이드바가 보여도 개별 숨김+바로가기 ON 인 항목이 있으면). 스튜디오 버튼을 감싼
// 박스(_box_) 앞에 두며, 없으면 헤더 첫 section 앞에 둔다. React 재렌더로 사라질 수
// 있어 init/옵저버에서 멱등 재호출.
function ensureHeaderNav() {
  const header = document.getElementById("header");
  if (!header) return;

  const shouldShow = HEADER_NAV_ITEMS.some(isHeaderNavItemVisible);
...
function normalizeFollowPreviewHeaderFont(v) {
  const n = Number(v);
  if (!Number.isFinite(n)) return FOLLOW_PREVIEW_HEADER_FONT_DEFAULT;
  return Math.min(
    FOLLOW_PREVIEW_HEADER_FONT_MAX,
    Math.max(FOLLOW_PREVIEW_HEADER_FONT_MIN, n),
  );
}

// 헤더(메타바) 폰트 배율을 컨테이너 CSS 변수로 설정 → CSS가 하위 폰트를 스케일.
function applyFollowPreviewHeaderFont(el) {
  const root = el || document.getElementById("cheese-follow-preview");
  if (!root) return;
  root.style.setProperty(
    "--cheese-fp-header-font",
    String(normalizeFollowPreviewHeaderFont(followPreviewHeaderFont)),
  );
}
...
// ══ 전용 팔로잉 목록(sbFollowCustom) 렌더/주입 ═════════════════════════════
const CUSTOM_FOLLOW_NAV_ID = "cheese-custom-follow";
...
// 잡은 '부분본'(프로필·inner 만)을 완전본이 나중에 덮어쓰도록 구분한다.
let customFollowHarvestComplete = false;
function harvestCustomFollowClasses(nav) {
  // 이미 완전본을 캐시했으면 재사용. 부분본만 있으면 완전본을 찾으러 계속 진행한다.
  if (customFollowHarvest && customFollowHarvestComplete)
    return customFollowHarvest;
  // ⚠ 우리 ul(.cheese-cf-list) 은 제외하고 '원본' ul 을 고른다.
  const ul = [...nav.querySelectorAll('ul[class*="_list_"]')].find(
    (u) => !u.classList.contains("cheese-cf-list"),
  );
  const li = ul?.querySelector(":scope > li");
  if (!ul || !li) return customFollowHarvest; // 원본이 비면 기존(부분/없음) 유지.
  // ⚠ 접힘 상태 원본 li 엔 _information_/_name_/_count_ 가 없다(아이콘만). 예전엔 그 경우
  // harvest 를 아예 건너뛰어 접힘 첫 진입이 폴백으로 그려졌다. 이제는 '있는 것만'(프로필·
  // inner·li) 부분 harvest 하고, 정보 영역까지 있는 완전본이 나오면 그때 덮어쓴다.
  const isComplete = !!li.querySelector('[class*="_information_"]');
  // 이미 부분본이 있고 이번에도 완전본이 아니면(둘 다 부분) 재계산 불필요 — 기존 유지.
  if (customFollowHarvest && !isComplete) return customFollowHarvest;
  // 우리 마커 클래스는 harvest 결과에서 제거(우리 요소에 섞여 숨김/오작동 방지).
  const clean = (cls) =>
    (cls || "")
      .split(/\s+/)
      .filter((c) => c && !c.startsWith("cheese-"))
      .join(" ");
  // 네이티브 li 내부 구조: li > div._item_ > (._profile_, ._information_ > ._name_ +
  // ._description_, em._count_, a._item_link_). 우리도 동일 구조 + 클래스를 harvest 해
  // 네이티브 CSS 를 그대로 얹는다.
  const inner = li.querySelector('[class*="_item_"][class*="_type_profile_"]');
  // ⚠ 프로필 아바타는 _profile_2us73_ 이지만 inner 도 _type_profile_ 를 가져 [class*="_profile_"]
  // 첫 매칭이 inner 를 잡는다. inner 자신은 제외하고, _type_ 없는 실제 프로필만 고른다.
  const profileEl = [
    ...(inner || li).querySelectorAll('[class*="_profile_"]'),
  ].find((el) => el !== inner && !/_type_profile_/.test(el.className));
  const profileLiveCls = clean(profileEl?.className);
  const nameEl = li.querySelector('[class*="_name_"]');
  const ellipsisEl = nameEl?.querySelector('[class*="_ellipsis_"]');
  const textEl = ellipsisEl?.querySelector('[class*="_text_"]');
  // inner 클래스에서 _is_expanded_ 계열을 분리(접힘 상태 대응). innerExpanded 는 펼침일 때만 부여.
  const innerCleanAll = clean(inner?.className) || "";
  const innerExpanded = (
    innerCleanAll.match(/\S*_is_expanded_\S*/g) || []
  ).join(" ");
  const result = {
    ul: clean(ul.className),
    li: clean(li.className),
    inner: innerCleanAll.replace(/\S*_is_expanded_\S*/g, "").trim(),
    innerExpanded,
    profile: profileLiveCls.replace(/\S*_is_live_\S*/g, "").trim(),
    profileLive: profileLiveCls,
    information: clean(li.querySelector('[class*="_information_"]')?.className),
  ...
  // _is_expanded_ 는 렌더 시 실시간 조회(getCustomFollowExpandedNow)하므로, harvest 는
  // 나머지 정적 클래스만 담는다. 완전본(정보 영역 포함)이면 complete 로 표시해, 이후 완전본이
  // 다시 나와도 재계산하지 않게 한다(부분본은 완전본이 나오면 덮어쓰기 허용).
  customFollowHarvest = result;
  customFollowHarvestComplete = isComplete;
  return result;
}
...
function createCustomFollowItemHtml(item, h, expandedNow) {
  ...
  const officialHtml = item.verifiedMark
    ? `<i class="${iconCls}" style="width:13px;height:13px" aria-hidden="true"></i><span class="blind">인증 마크</span>`
    : "";
  // 업적 배지는 첫 1개만 표시(네이티브 근사 + 폭 절약).
  const firstBadge = (item.achievementBadgeUrls || [])[0];
  const badgeHtml = firstBadge
    ? `<i class="${iconCls}" style="width:13px;height:13px;background-image:url('${escapeAttribute(firstBadge)}')" aria-hidden="true"></i>`
    : "";
  const countHtml =
    live && item.countText
      ? `<em class="${c(h?.count, "cheese-cf-count")}">${escapeHtml(item.countText)}</em>`
      : "";
  // _is_expanded_ 는 renderCustomFollowList 가 '실시간 조회'해 expandedNow 로 넘긴다
  ...
  return (
    ...
    (imageUrl
      ...
      : "") +
    `<span class="blind">${live ? "LIVE" : "오프라인"}</span>` +
    `</div>` +
    `<div class="${c(h?.information, "cheese-cf-info")}">` +
    `<strong class="${c(h?.name, "cheese-cf-name")}">` +
    `<span class="${c(h?.ellipsis, "cheese-cf-ellipsis")}">` +
    `<span class="${c(h?.text, "cheese-cf-text")}">${escapeHtml(item.name)}</span>` +
    officialHtml +
    badgeHtml +
    `</span>` +
    `</strong>` +
    (live && item.category
      ? `<span class="${c(h?.description, "cheese-cf-desc")}">${escapeHtml(item.category)}</span>`
      : "") +
    `</div>` +
    countHtml +
...
function customFollowSig(visibleCount, shown) {
  ...
  return [
    customFollowVersion,
    customFollowSort,
    featureFlags.sbFollowFavSort ? customFollowFavSort : "-",
    featureFlags.sbFollowFavSort && customFollowFavSort === "custom"
      ? customFollowFavOrder.join(",")
      : "-",
    favSig,
    shown,
    visibleCount,
...
function ensureCustomFollowList() {
  const sidebar = document.getElementById("sidebar");
  // 게이트: 전용 목록 마스터 ON + 사이드바 전체 숨김 아님. (sbFollow 섹션 숨김과는
  // 무관 — 전용 목록 사용 시 applySidebarSections 가 팔로우 nav 를 숨기지 않는다.)
  ...
  origNav.querySelectorAll('ul[class*="_list_"]').forEach((ul) => {
    if (!ul.classList.contains("cheese-cf-list"))
      ul.classList.add(CUSTOM_FOLLOW_ORIG_HIDE_CLASS);
  });
  // 원본 '더보기' 버튼 박스(_box_)도 숨김 — 우리 더보기와 중복 방지(재생성 대비 전부).
  origNav
    .querySelectorAll('[class*="_box_"]:has([class*="_more_button_"])')
  ...
  bindCustomFollowDelegation(); // document 위임 1회(노드 교체 무관)
  // 우리 nav 확보/생성(원본 nav 안, ul 뒤).
  let ourNav = document.getElementById(CUSTOM_FOLLOW_NAV_ID);
  if (!ourNav) {
    ourNav = document.createElement("div");
    ourNav.id = CUSTOM_FOLLOW_NAV_ID;
    ourNav.className = "cheese-cf-nav";
    origNav.appendChild(ourNav);
  } else if (ourNav.parentElement !== origNav) {
...
// 붙은 것). harvest 캐시 대신 매 렌더마다 읽어, 전환 직후 시차 없이 정확히 반영한다.
function getCustomFollowExpandedNow(origNav) {
  if (!isSidebarExpanded()) return "";
  const inner = origNav?.querySelector(
    'ul[class*="_list_"]:not(.cheese-cf-list) > li [class*="_is_expanded_"]',
  );
  const cls = (inner?.className || "").toString();
...
function bindCustomFollowDelegation() {
  ...
  customFollowDelegationBound = true;
  document.addEventListener(
    "click",
    (e) => {
      ...
      if (fav) {
        e.preventDefault();
        e.stopPropagation();
        // 접힘 상태(아이콘만)에선 별표는 '표시 전용' — 해제는 펼친 상태에서만 허용.
        if (isSidebarExpanded())
          toggleCustomFollowFavorite(fav.dataset.favId || "");
        return;
      }
...
// 기억하고, 다른 즐겨찾기 항목 위로 drop 하면 순서 배열을 재계산해 저장한다.
let customFollowDragId = "";
...
function bindCustomFollowDragSort() {
  ...
  const dragAfter = (listEl, y) => {
    const items = [
      ...listEl.querySelectorAll(
        ".cheese-cf-draggable:not(.cheese-cf-dragging)",
      ),
    ];
  ...
  const onStart = (e) => {
    ...
    if (e.dataTransfer) {
      ...
      try {
        e.dataTransfer.setData("text/plain", customFollowDragId);
      } catch {}
    }
    requestAnimationFrame(() => li.classList.add("cheese-cf-dragging"));
  ...
  const reorderToY = () => {
    ...
    if (after == null) {
      ...
      if (last && last.nextSibling !== customFollowDragEl)
        last.after(customFollowDragEl);
  ...
  // ── 꾹 누르기(long-press)로 드래그 모드 진입 ──────────────────────────────
  // draggable 은 렌더에서 기본 false. 항목을 LONGPRESS_MS 동안 누르고 있으면 draggable 을
  // 켜고 dragready 클래스를 부여(이때부터 드래그로 순서 편집). 그 전에 떼거나 크게 움직이면
  // 취소 → 짧은 클릭/탭은 링크 클릭(채널 이동)으로 흘러간다.
  const LONGPRESS_MS = 300;
  const MOVE_TOLERANCE = 8; // px: 이보다 크게 움직이면 스크롤/취소로 간주
  let pressTimer = 0;
  let pressLi = null;
  let pressX = 0;
  let pressY = 0;
  ...
  const onPointerDown = (e) => {
    ...
    pressX = e.clientX;
    pressY = e.clientY;
    cancelPressTimerOnly();
    pressTimer = setTimeout(() => {
      pressTimer = 0;
      if (!pressLi) return;
      // 드래그 준비: draggable 켜고 클래스 부여. 미리보기도 닫는다.
      clearDragReady();
      pressLi.setAttribute("draggable", "true");
      pressLi.classList.add("cheese-cf-dragready");
      if (typeof closeFollowPreview === "function") closeFollowPreview();
    }, LONGPRESS_MS);
  };
  function cancelPressTimerOnly() {
    if (pressTimer) clearTimeout(pressTimer);
    pressTimer = 0;
  }
  const onPointerMove = (e) => {
    if (!pressLi) return;
    if (
      Math.abs(e.clientX - pressX) > MOVE_TOLERANCE ||
      Math.abs(e.clientY - pressY) > MOVE_TOLERANCE
    ) {
      cancelPress(); // 크게 움직이면(스크롤 등) long-press 취소
    }
  };
  const onPointerUp = () => {
    // 실제 드래그(dragstart)로 이어지지 않았다면 dragready 를 곧 해제(클릭 이동 흐름 유지).
    cancelPress();
    // dragstart 가 안 났으면 다음 tick 에 dragready 해제(click 이 먼저 처리되게).
    setTimeout(() => {
      if (!customFollowDragEl) clearDragReady();
    }, 0);
  };
...
// 즐겨찾기 토글 + 저장 + 재렌더.
function toggleCustomFollowFavorite(channelId) {
  if (!channelId) return;
  if (customFollowFavorites.has(channelId)) {
    customFollowFavorites.delete(channelId);
  } else {
    customFollowFavorites.add(channelId);
  }
  try {
    chrome.storage?.local?.set({
      [CUSTOM_FOLLOW_FAVORITES_KEY]: [...customFollowFavorites],
    });
  } catch {}
...
// imageUrl} 를 저장한다(설정 팝업엔 팔로잉 데이터가 없으므로). 즐겨찾기 변경 시 갱신.
function saveCustomFollowFavMeta() {
  try {
    const byId = new Map(customFollowItems.map((it) => [it.channelId, it]));
    const meta = [...customFollowFavorites].map((id) => {
...
// side 미지정이면 targetId 바로 앞에 놓는다(기존 호출 호환).
function reorderCustomFollowFavorite(dragId, targetId, side) {
  if (!dragId || dragId === targetId) return;
  const order = currentFavoriteOrder();
  const from = order.indexOf(dragId);
  if (from < 0 || order.indexOf(targetId) < 0) return;
  order.splice(from, 1); // 먼저 드래그 항목을 뺀다
...
function ensureCustomFollowCollapsedControls() {
  const gateOn = featureFlags.sbFollowCustom && !featureFlags.sidebar;
  const origNav = findSidebarFollowNav();
  const existing = origNav?.querySelector("." + CUSTOM_FOLLOW_CTRL_CLASS);
  if (!gateOn || !origNav) {
    existing?.remove();
    return;
  }
...
async function fetchLivePreviewData(channelId) {
  ...
  try {
    ...
    if (!m3u8) return null;
    // 썸네일(라이브 스냅샷). liveImageUrl은 보통 {type} 플레이스홀더 포함 → 해상도 치환.
    const thumb = String(c?.liveImageUrl || "").replace("{type}", "1080");
...
async function openFollowPreview(li, channelId, anchorKind = "following") {
  ...
  if (followPreviewThumbOnly) {
    // 썸네일 모드: 영상 연결 없이 스냅샷 이미지만 표시(트래픽·소리 없음).
    teardownFollowPreviewMedia(
      el.querySelector(".cheese-follow-preview-video"),
    );
...
function renderFollowPreviewMeta(el, meta) {
  ...
  if (!bar || !meta) return;
  followPreviewState.lastMeta = meta; // 옵션 변경 시 즉시 재렌더용 보관
  ...
  bar.innerHTML =
    ...
    (meta.category
      ...
      : "") +
    `</span>` +
    // 2-1) 중앙 하단: 옵션이 켜지면 시청자/경과시간을 여기에도(창이 좁아 우측이 숨겨져도
    //      항상 보이게). meta-center 폭에 맞춰 한 줄(row)로 촘촘하게 표시.
    ((followPreviewAlwaysViewers && meta.viewers) ||
    (followPreviewAlwaysElapsed && elapsed)
      ? `<span class="cheese-follow-preview-meta-center-side">` +
        (followPreviewAlwaysViewers && meta.viewers
...
function startFollowPreviewElapsedTimer(el) {
  ...
  followPreviewState.elapsedTimer = setInterval(() => {
    // 우측(meta-side)과 중앙 하단(옵션) 복제본이 있을 수 있어 모두 갱신한다.
    el.querySelectorAll(".cheese-follow-preview-meta-elapsed").forEach(
      (span) => {
  ...
  void (async () => {
    ...
    if (typeof Hls !== "undefined" && Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (_e, data) => {
        ...
        const isKeyError = detail.includes("KEY") || detail.includes("keyLoad");
      ...
      if (followPreviewLiveEdge)
        hls.on(Hls.Events.LEVEL_LOADED, jumpToLiveEdge);
      hls.loadSource(m3u8);
...
async function loadFollowPreview() {
  ...
  try {
    ...
    followPreviewAlwaysElapsed =
      data?.[FOLLOW_PREVIEW_ALWAYS_ELAPSED_KEY] === true; // 기본 OFF
...
function clearCardDateTooltipHover() {
...
function onCardDateMouseOver(e) {
  ...
  // 다른 카드로 바로 이동했을 때 새 API 응답을 기다리는 동안 이전 카드 툴팁이 남지
  // 않도록 먼저 닫는다. Firefox에서 네트워크 지연이 있을 때 특히 오래 남아 보였다.
  if (cardDateHoverInfo !== info || cardDateHoverVideoNo !== videoNo) {
    hideCardDateTooltip();
  }
...
function onCardDateMouseOut(e) {
  const info = cardDateHoverInfo;
  if (!info || !e.target || !info.contains(e.target)) return;
  const next = e.relatedTarget;
  if (next instanceof Node && info.contains(next)) return;
  clearCardDateTooltipHover();
}

function onCardDateDocumentLeave() {
  clearCardDateTooltipHover();
}

function onCardDateVisibilityChange() {
  if (document.hidden) clearCardDateTooltipHover();
}
...
function bindCardDateTooltip() {
  ...
  document.addEventListener("mouseout", onCardDateMouseOut, {
    passive: true,
  });
  document.documentElement.addEventListener(
    "mouseleave",
    onCardDateDocumentLeave,
    { passive: true },
  );
  document.addEventListener("visibilitychange", onCardDateVisibilityChange);
...
function unbindCardDateTooltip() {
  ...
  document.removeEventListener("mouseout", onCardDateMouseOut);
  document.documentElement.removeEventListener(
    "mouseleave",
    onCardDateDocumentLeave,
  );
  document.removeEventListener("visibilitychange", onCardDateVisibilityChange);
...
async function fetchFollowChannelData(channelId) {
  ...
  try {
    const [history, myInfo, liveStatus] = await Promise.all([
      getJson(
        `${base}/service/v1/channels/${channelId}/data?fields=channelHistory`,
      ),
      getJson(`${base}/service/v1.1/channels/${channelId}/my-info`),
      getJson(`${base}/polling/v3.1/channels/${channelId}/live-status`),
    ]);
    const status = liveStatus?.content?.status || null; // OPEN | CLOSE
    const openDate = liveStatus?.content?.openDate || null;
    // 최근 방송일: 방송 중이면 openDate, 아니면 openDate → 최신 영상 → 최신 클립 순으로
    // 보강한다(정리 도구와 동일 로직). recentLiveMs 를 표시에 사용한다.
    let recentLiveMs = parseCleanupDateStr(openDate);
    if (status !== "OPEN" && !Number.isFinite(recentLiveMs)) {
      const videos = await getJson(
        `${base}/service/v1/channels/${channelId}/videos?sortType=LATEST&pagingType=PAGE&page=0&size=1&publishDateAt=&videoType=`,
      );
      const v = videos?.content?.data?.[0];
      if (v && Number.isFinite(Number(v.publishDateAt))) {
        recentLiveMs = Number(v.publishDateAt);
      } else {
        const clips = await getJson(
          `${base}/service/v1/channels/${channelId}/clips?clipUID=&filterType=ALL&orderType=RECENT&size=1`,
        );
        const cd = parseCleanupDateStr(clips?.content?.data?.[0]?.createdDate);
        if (Number.isFinite(cd)) recentLiveMs = cd;
      }
    }
    const data = {
      ...
      liveStatus: status,
      openDate,
      recentLiveMs: Number.isFinite(recentLiveMs) ? recentLiveMs : null,
    };
...
function followChannelTooltipHtml(data) {
  if (!data) return "";
  // 최근 방송일: 라이브 중이면 '방송 중', 아니면 openDate→최신 영상→최신 클립으로 보강한
  // recentLiveMs(epoch ms)를 표시. 없으면 '기록 없음'.
  let recent;
  if (data.liveStatus === "OPEN") {
    const openTxt = data.openDate ? fmtFollowChannelDate(data.openDate) : "";
    recent = openTxt ? `방송 중 (${openTxt}~)` : "방송 중";
  } else if (Number.isFinite(data.recentLiveMs)) {
    recent = fmtCleanupDate(Number(data.recentLiveMs));
  } else {
    recent = "기록 없음";
  }
  ...
  return (
    ...
    row(
      "팔로우 시작일",
      data.followDate ? fmtFollowChannelDate(data.followDate) : "—",
    ) +
...
const FOLLOW_CHANNEL_TOOLTIP_HOVER_DELAY_MS = 250;
function onFollowChannelMouseOver(e) {
  ...
  // 마우스가 스치기만 해도 즉시 API(최대 5개)를 쏘면, 목록을 훑을 때 요청이 폭증해
  // 페이지가 느려진다. 250ms 머무를 때만 fetch/표시(대기 중 벗어나면 취소).
  if (followChannelTooltipState.hoverTimer) {
    clearTimeout(followChannelTooltipState.hoverTimer);
  }
...
function onFollowChannelMouseOut(e) {
  ...
  // 대기 중이던 fetch 취소(스쳐 지나간 채널은 요청하지 않음).
  if (followChannelTooltipState.hoverTimer) {
    clearTimeout(followChannelTooltipState.hoverTimer);
    followChannelTooltipState.hoverTimer = 0;
  }
...
// 팔로잉 채널(툴팁 앵커)을 클릭해 이동하면 마우스가 그 자리에 남아 mouseout 이 안 나
// 툴팁이 계속 떠 있다. 앵커 클릭 시 즉시 툴팁을 숨긴다(헤더 자동 숨김 시 툴팁만 남던
// 문제 포함). 진행 중인 표시 대기도 취소한다.
function onFollowChannelClickHide(e) {
  if (!followChannelAnchorFromTarget(e.target)) return;
  followChannelTooltipState.hoverAnchor = null;
  followChannelTooltipState.currentChannelId = "";
  hideFollowChannelTooltip();
}
...
function bindFollowChannelTooltip() {
  ...
  document.addEventListener("click", onFollowChannelClickHide, true);
}
...
function unbindFollowChannelTooltip() {
  ...
  document.removeEventListener("click", onFollowChannelClickHide, true);
  followChannelTooltipState.hoverAnchor = null;
  followChannelTooltipState.currentChannelId = "";
  if (followChannelTooltipState.hoverTimer) {
    clearTimeout(followChannelTooltipState.hoverTimer);
...
// ══ 팔로잉 채널 정리 도구 ═══════════════════════════════════════════════════
// following?tab=CHANNEL 목록 앞에 '팔로잉 정리' 버튼을 넣고, 클릭 시 중앙 모달로 팔로잉
// 채널을 최근 방송일 필터(없음/1개월/3개월)로 모아 개별/선택/필터 전체 언팔로우한다.
const FOLLOW_CLEANUP_BTN_ID = "cheese-follow-cleanup-btn";
const FOLLOW_CLEANUP_MODAL_ID = "cheese-follow-cleanup-modal";
const FOLLOW_CLEANUP_API_BASE = "https://api.chzzk.naver.com";
const FOLLOW_CLEANUP_LASTLIVE_TTL_MS = 5 * 60 * 1000;
const followCleanupLastLiveCache = new Map(); // channelId → {info, at}
const followCleanupState = {
  open: false,
  loadToken: 0, // 모달 열 때마다 증가 — 이전 로드 취소용
  filter: "none", // none | m1 | m3
  channels: [], // [{channelId,name,imageUrl,followDate,openLive}]
  lastLive: new Map(), // channelId → {status:"LIVE"|"DATE"|"NONE", at?:ms}
  selected: new Set(), // 체크된 channelId
  running: false, // 언팔로우 진행 중
  lastLiveLoading: false, // 방송일 점진 로드 진행 중(끝나기 전엔 '없음' 대신 스켈레톤)
};

// following?tab=CHANNEL 페이지인지. tab 파라미터가 없으면 기본 탭이 CHANNEL 이 아닐 수
// 있어, 명시적으로 tab=CHANNEL 일 때만 대상으로 한다.
function isFollowingChannelPage() {
  if (location.pathname !== "/following") return false;
...
async function enrichCustomFollowOpenDates(items) {
  ...
  const worker = async () => {
    while (cursor < targets.length) {
      if (document.hidden) return; // 백그라운드면 중단
...
// 즐겨찾기 상단 고정을 반영한 정렬. mode 별 비교 + favorite 안정 분리.
// 한 그룹 내부의 mode 별 비교 함수.
function customFollowModeCmp(a, b, mode) {
  const liveRank = (it) => (it.live ? 0 : 1);
  const byName = (x, y) => x.name.localeCompare(y.name, "ko");
  switch (mode) {
    case "custom": {
      // 커스텀 순서: customFollowFavOrder 배열의 인덱스 순(없으면 뒤로 + 이름순).
      const ia = customFollowFavOrder.indexOf(a.channelId);
      const ib = customFollowFavOrder.indexOf(b.channelId);
    ...
    case "name-desc":
      return byName(b, a);
    case "recent": // 방송 시작 최근(라이브 우선 → openDate desc)
      if (liveRank(a) !== liveRank(b)) return liveRank(a) - liveRank(b);
      return (b.openDate || 0) - (a.openDate || 0) || byName(a, b);
    case "oldest": // 방송 시작 오래된 순(라이브 우선 → openDate asc)
      if (liveRank(a) !== liveRank(b)) return liveRank(a) - liveRank(b);
      return (
        (a.openDate || Infinity) - (b.openDate || Infinity) || byName(a, b)
      );
    case "popular":
    default: // 인기순(라이브 우선 → 시청자수 desc)
      if (liveRank(a) !== liveRank(b)) return liveRank(a) - liveRank(b);
...
// "YYYY-MM-DD HH:MM:SS" → epoch ms(로컬). 실패 시 NaN.
function parseCleanupDateStr(s) {
  return s ? Date.parse(String(s).replace(" ", "T")) : NaN;
}

// 최근 방송 시각 확보. 우선순위:
//  1) openLive=true → 방송 중.
//  2) 최신 영상(REPLAY 등)의 publishDateAt.
//  3) 최신 클립의 createdDate(다시보기 기록이 없어도 최근 활동 근사).
//  4) live-status openDate.
//  5) 없으면 NONE.
async function fetchChannelLastLive(channelId, openLive) {
  if (openLive) return { status: "LIVE" };
  const cached = followCleanupLastLiveCache.get(channelId);
  if (cached && Date.now() - cached.at < FOLLOW_CLEANUP_LASTLIVE_TTL_MS) {
    return cached.info;
  }
  const getJson = (url) =>
    fetch(url, {
      ...
      .then((r) => (r.ok ? r.json() : null))
      .catch(() => null);
  let info = { status: "NONE" };
  try {
    // 2) 최신 영상(다시보기).
    const videos = await getJson(
      `${FOLLOW_CLEANUP_API_BASE}/service/v1/channels/${channelId}/videos?sortType=LATEST&pagingType=PAGE&page=0&size=1&publishDateAt=&videoType=`,
    );
    const v = videos?.content?.data?.[0];
    if (v && Number.isFinite(Number(v.publishDateAt))) {
      info = { status: "DATE", at: Number(v.publishDateAt) };
    }
    // 3) 최신 클립(다시보기 없을 때).
    if (info.status === "NONE") {
      const clips = await getJson(
        `${FOLLOW_CLEANUP_API_BASE}/service/v1/channels/${channelId}/clips?clipUID=&filterType=ALL&orderType=RECENT&size=1`,
      );
      const clipDate = parseCleanupDateStr(
        clips?.content?.data?.[0]?.createdDate,
      );
      if (Number.isFinite(clipDate)) info = { status: "DATE", at: clipDate };
    }
    // 4) live-status openDate.
    if (info.status === "NONE") {
      const live = await getJson(
        `${FOLLOW_CLEANUP_API_BASE}/polling/v3.1/channels/${channelId}/live-status`,
      );
      const t = parseCleanupDateStr(live?.content?.openDate);
      if (Number.isFinite(t)) info = { status: "DATE", at: t };
    }
  } catch {
    info = { status: "NONE" };
  }
...
// ── 모달 ───────────────────────────────────────────────────────────────────
function openFollowCleanupModal() {
  if (followCleanupState.open) return;
  followCleanupState.open = true;
  followCleanupState.filter = "none"; // 기본: 기록 없음
  followCleanupState.selected = new Set();
  followCleanupState.channels = [];
  followCleanupState.lastLive = new Map();
  followCleanupState.running = false;
  followCleanupState.lastLiveLoading = true; // 열자마자 로딩 상태(스켈레톤 유지)
  const token = ++followCleanupState.loadToken;

  const filterOptions = FOLLOW_CLEANUP_FILTERS.map(
    (f) =>
      `<button type="button" class="cheese-fcln-filter-option${f.value === "none" ? " is-active" : ""}" role="option" data-filter="${f.value}">${escapeHtml(f.label)}</button>`,
  ).join("");

  const overlay = document.createElement("div");
  overlay.id = FOLLOW_CLEANUP_MODAL_ID;
  overlay.className = "cheese-follow-cleanup-overlay";
  overlay.innerHTML = `
    <div class="cheese-follow-cleanup-dialog" role="dialog" aria-label="팔로잉 정리" aria-modal="true">
      ...
      <div class="cheese-fcln-toolbar">
        <div class="cheese-fcln-filter">
          <span class="cheese-fcln-filter-label">최근 방송일</span>
          <div class="cheese-fcln-filter-picker">
            <button type="button" class="cheese-fcln-filter-trigger" aria-haspopup="listbox" aria-expanded="false">
              <span class="cheese-fcln-filter-current">${escapeHtml(followCleanupFilterLabel("none"))}</span>
              <svg class="cheese-fcln-filter-caret" width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true"><path d="M3 4.5L6 7.5L9 4.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
            <div class="cheese-fcln-filter-pop" role="listbox" hidden>${filterOptions}</div>
          </div>
        </div>
        <span class="cheese-fcln-summary"></span>
      </div>
      <div class="cheese-fcln-selectbar">
        <button type="button" class="cheese-fcln-selectall">전체 선택</button>
      </div>
      <div class="cheese-fcln-list" role="list"></div>
      <div class="cheese-fcln-progress" hidden></div>
      <div class="cheese-fcln-actions">
        <button type="button" class="cheese-fcln-unfollow-selected" disabled>선택 언팔로우</button>
        <button type="button" class="cheese-fcln-unfollow-filtered">필터 전체 언팔로우</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
  document.documentElement.classList.add("cheese-fcln-lock");

  // 닫기 핸들러(딤머/버튼/ESC).
  overlay.addEventListener("click", (e) => {
    if (e.target === overlay) closeFollowCleanupModal();
  });
  overlay
    .querySelector(".cheese-fcln-close")
    ?.addEventListener("click", closeFollowCleanupModal);
  document.addEventListener("keydown", onFollowCleanupKeydown, true);

  // 커스텀 필터 팝오버(열림/닫힘 애니메이션 — .is-open 클래스로 CSS transition).
  const trigger = overlay.querySelector(".cheese-fcln-filter-trigger");
  const pop = overlay.querySelector(".cheese-fcln-filter-pop");
  if (pop) pop.hidden = false; // 항상 렌더(표시는 클래스로 제어)
  const closePop = () => {
  ...
  pop?.addEventListener("click", (e) => {
    ...
    if (!opt) return;
    followCleanupState.filter = opt.dataset.filter;
    const cur = overlay.querySelector(".cheese-fcln-filter-current");
    if (cur)
      cur.textContent = followCleanupFilterLabel(followCleanupState.filter);
    pop
      .querySelectorAll("[data-filter]")
      .forEach((b) =>
        b.classList.toggle(
          "is-active",
          b.dataset.filter === followCleanupState.filter,
        ),
      );
    closePop();
    renderFollowCleanupList();
  });
  // 팝오버 바깥 클릭 시 닫기.
  overlay.addEventListener("click", (e) => {
  ...
  overlay.querySelector(".cheese-fcln-list")?.addEventListener("click", (e) => {
    ...
    if (e.target.closest("input[type=checkbox]")) {
      if (e.target.checked) followCleanupState.selected.add(cid);
      else followCleanupState.selected.delete(cid);
      row.classList.toggle("is-selected", e.target.checked);
      updateFollowCleanupSummary();
    }
  });
  overlay
    .querySelector(".cheese-fcln-selectall")
    ?.addEventListener("click", () => {
      const visible = getFollowCleanupVisibleChannels().map((c) => c.channelId);
      const allSel =
        visible.length > 0 &&
        visible.every((id) => followCleanupState.selected.has(id));
      if (allSel)
        visible.forEach((id) => followCleanupState.selected.delete(id));
      else visible.forEach((id) => followCleanupState.selected.add(id));
      renderFollowCleanupList();
    });
  overlay
    .querySelector(".cheese-fcln-unfollow-selected")
    ?.addEventListener("click", () => {
      runUnfollow([...followCleanupState.selected]);
    });
  overlay
    .querySelector(".cheese-fcln-unfollow-filtered")
    ?.addEventListener("click", () => {
      runUnfollow(getFollowCleanupVisibleChannels().map((c) => c.channelId));
    });

  // 목록 로드 + 방송일 점진 로드(정렬/필터 모두 방송일 필요).
  renderFollowCleanupList(); // 로딩 표시
  (async () => {
    try {
      const channels = await fetchAllFollowings();
      if (followCleanupState.loadToken !== token) return; // 그새 닫힘
      followCleanupState.channels = channels;
      renderFollowCleanupList();
      updateFollowCleanupSummary();
      startFollowCleanupLastLiveLoad();
    } catch {
      const list = overlay.querySelector(".cheese-fcln-list");
      if (list && followCleanupState.loadToken === token) {
        list.innerHTML =
          '<div class="cheese-fcln-empty">목록을 불러오지 못했습니다.</div>';
      }
    }
  })();
}

function onFollowCleanupKeydown(e) {
  if (e.key === "Escape" && followCleanupState.open) {
    e.preventDefault();
    closeFollowCleanupModal();
  }
}

function closeFollowCleanupModal() {
  followCleanupState.open = false;
  followCleanupState.loadToken++; // 진행 중 로드 취소
  document.removeEventListener("keydown", onFollowCleanupKeydown, true);
  document.documentElement.classList.remove("cheese-fcln-lock");
  document.getElementById(FOLLOW_CLEANUP_MODAL_ID)?.remove();
}

// 필터 옵션 정의(라벨/개월). all=전체, none=기록 없음, mN/yN=최근 N 이전.
const FOLLOW_CLEANUP_FILTERS = [
  { value: "none", label: "기록 없음", months: null },
  { value: "m1", label: "최근 1개월 이전", months: 1 },
  { value: "m3", label: "최근 3개월 이전", months: 3 },
  { value: "m6", label: "최근 6개월 이전", months: 6 },
  { value: "y1", label: "최근 1년 이전", months: 12 },
  { value: "all", label: "전체", months: 0 },
];
function followCleanupFilterLabel(value) {
  return (
    FOLLOW_CLEANUP_FILTERS.find((f) => f.value === value)?.label || "기록 없음"
  );
}

// 채널의 최근 방송 정렬 키(ms). 방송 중=매우 큼(맨 위), DATE=at, NONE=0.
function followCleanupSortMs(c) {
  if (c.openLive) return Number.MAX_SAFE_INTEGER;
  const ll = followCleanupState.lastLive.get(c.channelId);
  if (!ll) return -1; // 아직 미확정
  if (ll.status === "LIVE") return Number.MAX_SAFE_INTEGER;
  if (ll.status === "DATE") return Number(ll.at) || 0;
  return 0; // NONE
}

// 필터 기준 표시 대상 + 정렬(최근 방송일 최신순, 동일하면 채널명 오름차순).
function getFollowCleanupVisibleChannels() {
  const f = followCleanupState.filter;
  const def = FOLLOW_CLEANUP_FILTERS.find((x) => x.value === f);
  const months = def?.months;
  const filtered = followCleanupState.channels.filter((c) => {
    if (f === "all") return true; // 전체
    if (c.openLive) return false; // 방송 중은 정리 후보 아님
    const ll = followCleanupState.lastLive.get(c.channelId);
    if (!ll) return false; // 아직 로딩 전 → 확정되면 재평가
    if (ll.status === "LIVE") return false;
    if (f === "none") return ll.status === "NONE"; // 기록 없음만
    // mN/yN: 방송일이 기준보다 오래된 것(기록 없음도 '오래됨'으로 포함).
    if (ll.status === "NONE") return true;
    const cutoff = Date.now() - months * 30 * 24 * 60 * 60 * 1000;
    return ll.status === "DATE" && Number(ll.at) < cutoff;
  });
  return filtered.sort((a, b) => {
    const d = followCleanupSortMs(b) - followCleanupSortMs(a); // 최신 먼저
    if (d !== 0) return d;
    return a.name.localeCompare(b.name, "ko"); // 동일 방송일 → 이름 오름차순
  });
}

function followCleanupLastLiveText(channel) {
  if (channel.openLive) return "방송 중";
  const ll = followCleanupState.lastLive.get(channel.channelId);
  if (!ll) return "…";
  if (ll.status === "LIVE") return "방송 중";
  if (ll.status === "DATE") return fmtCleanupDate(Number(ll.at));
  return "기록 없음";
}

function followCleanupSkeletonHtml() {
  // 로드 전/방송일 로딩 중: 스켈레톤 행(체크·프로필·이름·날짜·버튼 자리).
  return Array.from({ length: 8 })
    .map(
      () =>
        `<div class="cheese-fcln-row cheese-fcln-skeleton" aria-hidden="true">
          <span class="cheese-fcln-sk cheese-fcln-sk-check"></span>
          <span class="cheese-fcln-sk cheese-fcln-sk-prof"></span>
          <span class="cheese-fcln-sk cheese-fcln-sk-name"></span>
          <span class="cheese-fcln-sk cheese-fcln-sk-date"></span>
          <span class="cheese-fcln-sk cheese-fcln-sk-btn"></span>
        </div>`,
    )
    .join("");
}

function renderFollowCleanupList() {
  const overlay = document.getElementById(FOLLOW_CLEANUP_MODAL_ID);
  if (!overlay) return;
  const list = overlay.querySelector(".cheese-fcln-list");
  if (!list) return;
  // 팔로잉 목록조차 아직 없으면 스켈레톤.
  if (followCleanupState.channels.length === 0) {
    list.innerHTML = followCleanupSkeletonHtml();
    updateFollowCleanupSummary();
    return;
  }
  const visible = getFollowCleanupVisibleChannels();
  if (visible.length === 0) {
    // 방송일 점진 로드가 아직 진행 중이면(필터 판정 미확정) '없음' 대신 스켈레톤 유지.
    // 로드가 끝났는데도 비었으면 진짜 '없음'.
    list.innerHTML = followCleanupState.lastLiveLoading
      ? followCleanupSkeletonHtml()
      : '<div class="cheese-fcln-empty">조건에 맞는 채널이 없습니다.</div>';
    updateFollowCleanupSummary();
    return;
  }
  list.innerHTML = visible
    .map((c) => {
      const isSel = followCleanupState.selected.has(c.channelId);
      const checked = isSel ? "checked" : "";
      const imgUrl = c.imageUrl
        ? `${c.imageUrl}${c.imageUrl.includes("?") ? "&" : "?"}type=f60_60_na`
        : "";
      const img = imgUrl
        ? `<img src="${escapeAttribute(imgUrl)}" alt="" loading="lazy" />`
        : '<span class="cheese-fcln-noimg"></span>';
      return `<div class="cheese-fcln-row${isSel ? " is-selected" : ""}" role="listitem" data-cid="${escapeAttribute(c.channelId)}">
        <label class="cheese-fcln-check"><input type="checkbox" ${checked} /></label>
        <span class="cheese-fcln-prof">${img}</span>
        <span class="cheese-fcln-name">${escapeHtml(c.name)}</span>
        <span class="cheese-fcln-date">${escapeHtml(followCleanupLastLiveText(c))}</span>
        <button type="button" class="cheese-fcln-row-unfollow">언팔로우</button>
      </div>`;
    })
    .join("");
  updateFollowCleanupSummary();
...
function updateFollowCleanupSummary() {
  const overlay = document.getElementById(FOLLOW_CLEANUP_MODAL_ID);
  if (!overlay) return;
  const visible = getFollowCleanupVisibleChannels();
  const sel = followCleanupState.selected.size;
  const summary = overlay.querySelector(".cheese-fcln-summary");
  if (summary) summary.textContent = `표시 ${visible.length} · 선택 ${sel}`;
  const selBtn = overlay.querySelector(".cheese-fcln-unfollow-selected");
  if (selBtn) selBtn.disabled = sel === 0 || followCleanupState.running;
  const filtBtn = overlay.querySelector(".cheese-fcln-unfollow-filtered");
  if (filtBtn)
    filtBtn.disabled = visible.length === 0 || followCleanupState.running;
  // 전체 선택/해제 토글: 표시된 채널이 모두 선택돼 있으면 '전체 해제', 아니면 '전체 선택'.
  const allBtn = overlay.querySelector(".cheese-fcln-selectall");
  if (allBtn) {
    const allSelected =
      visible.length > 0 &&
      visible.every((c) => followCleanupState.selected.has(c.channelId));
    allBtn.textContent = allSelected ? "전체 해제" : "전체 선택";
  }
}

// ── 점진적 최근 방송일 로드(동시성 5, 취소 가능) ───────────────────────────
// 정렬·모든 필터가 방송일을 필요로 하므로 항상 로드한다(전체 목록도 최신순 정렬).
async function startFollowCleanupLastLiveLoad() {
  const token = followCleanupState.loadToken;
  followCleanupState.lastLiveLoading = true; // 로딩 시작(끝나기 전 '없음' 숨김)
  // 아직 방송일 미확정인 채널만 대상.
  const pending = followCleanupState.channels.filter(
    (c) => !c.openLive && !followCleanupState.lastLive.has(c.channelId),
  );
  // openLive 채널은 즉시 LIVE 로 확정.
  followCleanupState.channels.forEach((c) => {
    if (c.openLive && !followCleanupState.lastLive.has(c.channelId)) {
      followCleanupState.lastLive.set(c.channelId, { status: "LIVE" });
    }
  });
  const CONCURRENCY = 5;
  let idx = 0;
  async function worker() {
    while (idx < pending.length) {
      if (followCleanupState.loadToken !== token) return; // 취소
      const c = pending[idx++];
      const info = await fetchChannelLastLive(c.channelId, c.openLive);
      if (followCleanupState.loadToken !== token) return;
      followCleanupState.lastLive.set(c.channelId, info);
      // 완료되는 대로 갱신(필터 재평가 포함). 잦은 렌더 완화를 위해 몇 개마다 렌더.
      if (idx % CONCURRENCY === 0 || idx === pending.length) {
        renderFollowCleanupList();
      }
    }
  }
  ...
  if (followCleanupState.loadToken === token) {
    followCleanupState.lastLiveLoading = false; // 로딩 완료 → 이제 '없음' 표시 허용
    renderFollowCleanupList();
  }
}
...
async function unfollowChannel(channelId) {
  const res = await fetch(
    `${FOLLOW_CLEANUP_API_BASE}/service/v1/channels/${channelId}/follow`,
    {
      method: "DELETE",
      credentials: "include",
      headers: { accept: "application/json" },
...
// 커스텀 확인 팝업(모달 위에 겹쳐 표시). Promise<boolean> 반환.
function followCleanupConfirm(message) {
  return new Promise((resolve) => {
    const wrap = document.createElement("div");
    wrap.className = "cheese-fcln-confirm-overlay";
    wrap.innerHTML = `
      <div class="cheese-fcln-confirm" role="alertdialog" aria-modal="true">
        <p class="cheese-fcln-confirm-msg">${escapeHtml(message)}</p>
        <div class="cheese-fcln-confirm-actions">
          <button type="button" class="cheese-fcln-confirm-cancel">취소</button>
          <button type="button" class="cheese-fcln-confirm-ok">언팔로우</button>
        </div>
      </div>`;
    const host =
      document.getElementById(FOLLOW_CLEANUP_MODAL_ID) || document.body;
    ...
    wrap
      .querySelector(".cheese-fcln-confirm-cancel")
      ?.addEventListener("click", () => done(false));
    wrap
      .querySelector(".cheese-fcln-confirm-ok")
      ?.addEventListener("click", () => done(true));
  });
}

async function runUnfollow(channelIds) {
  const ids = [...new Set(channelIds)].filter(Boolean);
  if (ids.length === 0 || followCleanupState.running) return;
  const ok = await followCleanupConfirm(
    `${ids.length}개 채널을 언팔로우합니다. 계속할까요?`,
  );
  if (!ok) return;
  followCleanupState.running = true;
  updateFollowCleanupSummary();
  const overlay = document.getElementById(FOLLOW_CLEANUP_MODAL_ID);
  const progress = overlay?.querySelector(".cheese-fcln-progress");
  if (progress) {
    progress.hidden = false;
    progress.textContent = `언팔로우 중 0 / ${ids.length}`;
  }
  let done = 0;
  let failed = 0;
  const token = followCleanupState.loadToken;
  for (const cid of ids) {
    if (!followCleanupState.open || followCleanupState.loadToken !== token)
      break;
    let success = false;
    try {
      success = await unfollowChannel(cid);
    } catch {
      success = false;
    }
    done += 1;
    if (success) {
      // 상태에서 제거 → 목록에서 사라짐.
      followCleanupState.channels = followCleanupState.channels.filter(
        (c) => c.channelId !== cid,
      );
      followCleanupState.selected.delete(cid);
      followCleanupState.lastLive.delete(cid);
    } else {
      failed += 1;
    }
    if (progress) {
      progress.textContent = `언팔로우 중 ${done} / ${ids.length}${failed ? ` (실패 ${failed})` : ""}`;
    }
    renderFollowCleanupList();
    // rate-limit 방지 간격.
  ...
  followCleanupState.running = false;
  if (progress) {
    progress.textContent = `완료 — 언팔로우 ${done - failed}개${failed ? `, 실패 ${failed}개` : ""}`;
    window.setTimeout(() => {
      if (progress && !followCleanupState.running) progress.hidden = true;
    }, 4000);
  }
  updateFollowCleanupSummary();
}
...
async function loadChatFoldPersist() {
  if (!chrome.storage?.local) return;
  try {
    const data = await getBootData([CHAT_FOLD_PERSIST_KEY]);
    chatFoldPersistOn = data?.[CHAT_FOLD_PERSIST_KEY] === true; // 기본 OFF
  } catch {}
...
async function loadHeaderNav() {
  if (!chrome.storage?.local) return;
  try {
    const data = await getBootData([HEADER_NAV_KEY, HEADER_FOLLOW_COUNT_KEY]);
...
function clickFollowRefresh() {
  if (document.hidden) return;
  // 전용 팔로잉 목록이 켜져 있으면 이 기존 팔로우 갱신은 '비활성화'한다(전용 목록은
  // 자체 주기 타이머 cheeseFollowCustomRefreshSec 로 갱신). 사이드바가 보이는 상태에서만
  // 해당(숨김이면 헤더 팔로잉은 아래 분기가 처리). 헤더 팔로잉 캐러셀 갱신은 계속 허용.
  if (featureFlags.sbFollowCustom && !featureFlags.sidebar) {
    return;
  }
  if (
    featureFlags.sidebar &&
...
// ── 전용 팔로잉 목록 자동 갱신(별도 주기) ──────────────────────────────────
// 전용 목록은 우리가 직접 refreshCustomFollowList 를 주기 호출한다(네이티브 버튼 클릭이
// 아니므로 별도 주기). 전용 목록 사용 시엔 기존 팔로우 갱신을 clickFollowRefresh 에서
// 비활성화하고, 이 타이머만 동작한다.
const CUSTOM_FOLLOW_REFRESH_KEY = "cheeseFollowCustomRefreshSec";
let customFollowRefreshSec = 0;
...
function stopCustomFollowRefreshTimer() {
  if (customFollowRefreshTimer) {
    clearTimeout(customFollowRefreshTimer);
    customFollowRefreshTimer = 0;
  }
}
function applyCustomFollowRefresh(secRaw) {
  let sec = Number(secRaw);
  if (!Number.isFinite(sec) || sec <= 0) sec = 0;
  else sec = Math.min(600, Math.max(3, sec));
  customFollowRefreshSec = sec;
  startCustomFollowRefreshTimer();
}
...
async function loadFollowRefresh() {
  ...
  try {
    ...
    applyCustomFollowRefresh(data?.[CUSTOM_FOLLOW_REFRESH_KEY]);
  } catch {}
}
...
function applyCustomFollowSettings(data) {
  ...
  customFollowSort = CUSTOM_FOLLOW_SORTS.includes(sort)
    ? sort
    : CUSTOM_FOLLOW_SORT_DEFAULT;
  const favSort = data?.[CUSTOM_FOLLOW_FAV_SORT_KEY];
  customFollowFavSort = CUSTOM_FOLLOW_FAV_SORTS.includes(favSort)
    ? favSort
    : CUSTOM_FOLLOW_SORT_DEFAULT;
  ...
  customFollowMore = clampCustomFollowCount(
...
if (chrome.storage?.onChanged) {
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (changes[AD_MINI_KEEP_MUTED_KEY]) {
      adMiniplayerKeepMuted =
        changes[AD_MINI_KEEP_MUTED_KEY].newValue !== false;
    }
    ...
    if (changes[WHEEL_VOLUME_RIGHTCLICK_KEY]) {
    ...
    if (changes[ACTION_OVERLAY_POS_KEY]) {
      actionOverlayPos = normalizeActionOverlayPos(
        changes[ACTION_OVERLAY_POS_KEY].newValue,
      );
      broadcastFeatureFlags();
    }
    ...
    if (changes[FOLLOW_PREVIEW_HEADER_FONT_KEY]) {
      followPreviewHeaderFont = normalizeFollowPreviewHeaderFont(
        changes[FOLLOW_PREVIEW_HEADER_FONT_KEY].newValue,
      );
      applyFollowPreviewHeaderFont(); // 현재 떠 있는 미리보기에 즉시 반영
    }
    if (changes[FOLLOW_PREVIEW_THUMB_KEY]) {
      followPreviewThumbOnly =
        changes[FOLLOW_PREVIEW_THUMB_KEY].newValue === true;
      const el = document.getElementById(FOLLOW_PREVIEW_ID);
      el?.classList.toggle("is-thumb-only", followPreviewThumbOnly);
      // 모드가 바뀌면 다음 호버부터 적용(현재 떠 있는 건 닫아 깔끔히 전환).
      if (el) closeFollowPreview();
    }
    if (changes[FOLLOW_PREVIEW_FULL_TITLE_KEY]) {
      followPreviewFullTitle =
        changes[FOLLOW_PREVIEW_FULL_TITLE_KEY].newValue === true;
      document
    ...
    if (
      ...
      if (changes[FOLLOW_PREVIEW_ALWAYS_VIEWERS_KEY])
        followPreviewAlwaysViewers =
          changes[FOLLOW_PREVIEW_ALWAYS_VIEWERS_KEY].newValue === true;
      if (changes[FOLLOW_PREVIEW_ALWAYS_ELAPSED_KEY])
        followPreviewAlwaysElapsed =
          changes[FOLLOW_PREVIEW_ALWAYS_ELAPSED_KEY].newValue === true;
      // 떠 있는 미리보기 메타 즉시 재렌더(보관된 마지막 meta 로).
    ...
    if (changes[CHAT_FOLD_PERSIST_KEY]) {
      chatFoldPersistOn = changes[CHAT_FOLD_PERSIST_KEY].newValue === true;
      if (chatFoldPersistOn) {
        // 방금 '켜는' 순간엔 옛 저장값으로 이 페이지를 복원하면 안 된다(접은 적 없는데
        // 자동으로 접히는 문제). 대신 현재 화면 상태를 저장 기준으로 삼고, 이 페이지에선
        // 복원을 건너뛴다(현재 key 를 복원됨으로 선점). 복원은 다음 진입부터 적용된다.
        if (location.pathname.startsWith("/live/") && getLiveChatAside()) {
          try {
            chrome.storage.local.set({
              [CHAT_FOLD_STATE_KEY]: isChatFolded(),
            });
          } catch {}
          chatFoldRestoredForKey = location.pathname; // 이 페이지 복원 스킵
        } else {
          chatFoldRestoredForKey = "";
        }
      } else {
        chatFoldRestoredForKey = ""; // 끄면 다음에 다시 켤 때 재복원 허용
      }
      applyChatFoldPersist();
    }
    ...
    if (changes[CUSTOM_FOLLOW_REFRESH_KEY]) {
      applyCustomFollowRefresh(changes[CUSTOM_FOLLOW_REFRESH_KEY].newValue);
    }
    ...
    // 전용 팔로잉 목록 설정 변경 → 값 반영 + sig 무효화 + 재렌더.
    if (
      changes[CUSTOM_FOLLOW_SORT_KEY] ||
      changes[CUSTOM_FOLLOW_FAV_SORT_KEY] ||
      ...
      applyCustomFollowSettings({
        ...
        [CUSTOM_FOLLOW_FAV_SORT_KEY]:
          changes[CUSTOM_FOLLOW_FAV_SORT_KEY]?.newValue ?? customFollowFavSort,
        [CUSTOM_FOLLOW_FAV_ORDER_KEY]: changes[CUSTOM_FOLLOW_FAV_ORDER_KEY]
          ?.newValue ?? [...customFollowFavOrder],
        [CUSTOM_FOLLOW_INITIAL_KEY]:
      ...
      if (
        (changes[CUSTOM_FOLLOW_SORT_KEY] ||
          changes[CUSTOM_FOLLOW_FAV_SORT_KEY]) &&
        (needsOpenDate(customFollowSort) ||
          (featureFlags.sbFollowFavSort &&
            needsOpenDate(customFollowFavSort))) &&
        customFollowItems.some((it) => it.live && !it.openDate)
      ) {
        void enrichCustomFollowOpenDates(customFollowItems).then(() => {
          customFollowVersion += 1;
          ensureCustomFollowList();
        });
      }
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  if (message?.type === "LOG_POWER_WATCH_REWARD_STATUS") {
    applyWatchRewardStatus(message);
    return;
  }
  if (message?.type === "LOG_POWER_LIVE_ENDED") {

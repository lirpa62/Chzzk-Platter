...
(async () => {
  ...
  // 남아 스크롤이 잠기는 경우가 있다. 우리 UI 는 이 값을 인라인으로 쓰지 않고
  ...
  //   그래서 게인을 올려도 음량이 거의 안 커지고 압축만 깊어져 소리가 먹먹해졌다.
  ...
  function buildGraph(video) {
    ...
    try {
      ...
      //   출력을 막지 못한다. 그대로 두면 플레이어에서 음소거해도 소리가 계속 났다.
  ...
  //   나고, 그게 "지직" 하는 노이즈·소리 튐으로 들린다(렉 걸릴 때 소리가 튄다).
  ...
  function snapshotChannelPreset() {
    return {
      ...
      //   그래서 게인만 바꿔도 다음 방문부터 툴팁이 '사용자 설정'으로만 나왔다.
  ...
  function mixerButtonLabel() {
    ...
    if (presetDirty) {
      ...
      // 게인만 다르면 '수정된 X' 대신 'X · 180%'로 차이를 바로 보여 준다.
      if (isGainOnlyDirty()) {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (btn) {
      ...
      // 브라우저의 '포커스된 버튼 활성화'로 click을 합성해 믹서가 다시 켜질 수 있다.
      // 해당 키보드 합성 클릭만 무시한다.
  ...
  // chrome.downloads는 일부 제어·서식 문자가 섞인 파일명을 거부하므로 금지문자
  // 치환과 함께 아래 항목도 제거한다.
  ...
  //   이모지가 문제였다. ZWJ 는 눈에 보이지 않지만 서식문자라
  ...
  // 실패 사유를 짧게 안내하되 알 수 없는 오류 코드는 진단을 위해 그대로 노출한다.
  ...
  function onScreenshotSaved(result, retryData) {
    ...
    // 파일명 오류는 사용자가 이름을 직접 정할 수 있도록 저장 대화상자로 재시도한다.
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      //   번거로운 곳에서 설정을 바꿀 방법이 없다(단축키가 안 먹는 것처럼 보임).
  ...
  // 다른 탭에서 돌아온 뒤 재생 위치가 버퍼 끝을 벗어나 seeking 상태가 지속될 수 있다.
  // 관찰된 상태:
  ...
  // ── 다시보기 재생 속도 ────────────────────────────────────────────────────
  // 설정 > 재생 속도를 두 단계 들어가야 하는 번거로움을 없앤다. 유튜브식
  ...
  //   조작에서는 그 사이 네이티브 툴팁이 그대로 고정돼 보였다.
  ...
  function createSpeedButton() {
    ...
    //   잡혀서, 맨 span 만 두면 툴팁이 버튼이 아닌 엉뚱한 곳에 뜬다.
    btn.innerHTML =
    ...
    //   툴팁도 그대로 띄우므로, 마우스를 떼도 강조·툴팁이 고정돼 보인다.
  ...
  function ensureSpeedButton() {
    ...
    //   새 버튼을 하나 더 만들었다(옮긴 자리에 빈 아이콘이 생김).
  ...
  function stopSyncCatchUp() {
    ...
    //    그러면 이 조건이 거짓이 되어 원복을 건너뛰고 배속이 걸린 채 남았다.
  ...
  // 메뉴를 먼저 차단한다.
  //
  // ⚠ 익명 함수로 한 번만 등록하지 않는다(등록 코드는 실행됐는데 실제 우클릭
  ...
  //   우리가 1.0 까지 올리면 슬라이더가 147% 가 되고 핸들이 라인 밖으로 나간다.
  ...
  function tick() {
    ...
    // 하므로 비용은 참조 비교 한 번). 등록은 됐는데 실제로는 리스너가 없었음.
    ensureContextMenuHandlers();
    ...
    if (!pageKey) {
      ...
      // 벗어난 뒤라 치지직의 해제 로직이 돌지 않아 스크롤이 잠긴다(재현 경로).
  ...
  function maybeAutoWideScreen() {
    ...
    // 사라진다(해당 증상). 로드가 끝난 뒤의 false 만 '꺼짐'으로 확정한다.

...
(async () => {
  ...
  function isBlindRestoreActive() {
    ...
    // 죽었다. 무한 재시도를 막으려던 목적은 복원 쪽에만 해당한다.
  ...
  //   저장될 수 있다. img의 title을 이모티콘 키로 사용해 {:키:} 형태로 복원한다.
  ...
  // 우리 기능이 영영 잠긴다('가려진 채팅 표시'가 계속 비활성).
  ...
  function processRow(
    ...
    //   행이 곧바로 done 처리돼 아래 일반 경로까지 내려오지 않는다(저장이 전혀
  ...
  function ensureChatRowObserver() {
    ...
    chatRowObserver = new MutationObserver((mutations) => {
      ...
      if (
        ...
        // ⚠ 여기서 옵저버를 멈추면 시간 표시까지 죽는다(오프라인 채널 문제의 원인).
  ...
  //   없을 수 있다(Alt+클릭이 안 먹힘). 그때는 이미지 URL 의 파일명을 쓴다 —
  ...
  //   그래서 설정에 키를 직접 적어도 채팅에서 찾을 수가 없었다.

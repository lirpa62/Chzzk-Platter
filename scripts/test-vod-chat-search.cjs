...
//  - 현재 보고 있는 영상 것만 메모리에 두고, 영상이 바뀌면 곧바로 버린다
//  - 같은 영상을 두 번 순회하지 않는다(활성도 순회를 함께 쓴다)
//
// ⚠ 예전에는 '검색을 연 뒤에만 모은다(opt-in)' 였다. 그러면 활성도를 먼저
//   수집한 사람이 검색을 열 때 같은 영상을 처음부터 다시 받아야 했다. 지금은
//   순회 도중 늘 모아 두고 끝날 때 넘긴다(추가 요청은 없다).
...
console.log("\n[소스] 정책이 코드에 실제로 들어 있다");
{
  ...
  // ⚠ 정책이 바뀌었다. 예전에는 검색을 연 뒤에만 원문을 모았는데(opt-in),
  //   그러면 활성도를 먼저 수집한 사람이 검색을 열 때 같은 영상을 두 번 받았다.
  //   이제는 순회 도중 늘 모아 두고 끝날 때 넘긴다. 추가 요청은 없다.
  //   대신 '현재 영상 것만 메모리에 둔다' 는 선은 그대로다(아래 [H] 참고).
  ok(
    /const searchMessages = \[\];/.test(src),
    "순회 중에 원문을 모은다(활성도·검색이 한 순회를 공유)",
  );
  // 같은 영상을 두 번 순회하지 않는다.
  ok(
